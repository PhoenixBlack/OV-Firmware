...
FUNCTION MM6_REENTRY_GUIDANCE {
	...
	IF ROLL_CMD < TGT_ROLL_CMD {
		SET ROLL_CMD TO MIN(TGT_ROLL_CMD, ROLL_CMD + MM6_ROLL_REV_SPEED*DT). // 6 for cs
	}
	IF ROLL_CMD > TGT_ROLL_CMD {
		SET ROLL_CMD TO MAX(TGT_ROLL_CMD, ROLL_CMD - MM6_ROLL_REV_SPEED*DT).
	}
...
FUNCTION MAJOR_MODE6 {
	...
	IF MADS_UPDATE {
		...
		MADS_TELEMETRY(8, ROUND(TGT_PITCH_CMD,2) ).
		MADS_TELEMETRY(9, ROUND(TGT_ROLL_CMD,2) ).
	}
}
