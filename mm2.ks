...
FUNCTION MAJOR_MODE2_TRANSFER {
	IF RESTART {
		...
		SET MM2_DOCK_DY TO 90.
		SET MM2_DOCK_DR TO -90.		
...
FUNCTION MAJOR_MODE2 {
	...
	IF (MINOR_MODE = 2) OR 
	   (MINOR_MODE = 3) OR 
	   (MINOR_MODE = 4) { // STATION-KEEPING/DOCKING
	...
	IF MINOR_MODE = 5 {	// TARGET TRACKING
		IF HASTARGET {
			SET REL_POSITION 		TO TARGET:POSITION - SHIP:POSITION.
			SET REL_POSITION_SHIP	TO (-SHIP:FACING) * REL_POSITION.
	...
	IF UI_UPDATE {
		...
		IF (MINOR_MODE = 2) OR (MINOR_MODE = 3) OR (MINOR_MODE = 4) {
