...
FUNCTION MAJOR_MODE3_TRANSFER {
	...
	SET APPROACH_CONTROLLER TO PIDLOOP(0.05, 0.0, 0.25,  -9.0, 12.5).
	SET GS_CONTROLLER TO PIDLOOP(0.5, 0.05, 0.75,  -9.0, 12.5).
	SET FLARE_CONTROLLER TO PIDLOOP(1.0, 0.0, 0.6, -2.0, 7.5).
...
FUNCTION MAJOR_MODE3 {
	...
	IF (MINOR_MODE = 1) OR (MINOR_MODE = 2) { // DESCENT/APPROACH
		IF (TOTAL_ENERGY > TARGET_ENERGY) OR (SHIP:Q > 0.19) {
			BRAKES ON.
		}
		IF (TOTAL_ENERGY < TARGET_ENERGY) AND (SHIP:Q < 0.19) {
			BRAKES OFF.
		}
		...
		// STEERING LOOP
		LOCK STEERING TO HEADING(90 + HNAV, -10 + VNAV). //*R(0,0,-HNAV).
	}
