...
IF MINOR_MODE >= 3 {
	...
	IF MINOR_MODE < 3 {
		...
		//UI_VARIABLE("  +X",	"M",	ROUND(EI_DOWNRANGE/100,0)*100, 		1,7, NUMBER,	0,12).
		//UI_VARIABLE("  DX",	"M/S",	DR_DT, 								1,7, SIGNED,	0,13).
		//UI_VARIABLE("  +Z",	"M",	SHIP:BODY:ALTITUDEOF(POSITIONAT(SHIP,EI_ESTIMATE)), 0,7, NUMBER,	0,14).
	} ELSE {
	//IF T < 1000.0 {
	//	UI_VARIABLE("TIME",	"SEC",	T, 									2,7, NUMBER,	0,11).
	//} ELSE {
	//	UI_VARIABLE("TIME",	"SEC",	"-----",							0,7, TEXT,		0,11).
	//}
		//UI_VARIABLE("TREF",	"SEC",	T0, 								2,7, NUMBER,	0,12).
		UI_VARIABLE("DRAG",	"M/S2",	DRAG_CUR, 							2,7, SIGNED,	0,13).
		UI_VARIABLE("VERT",	"M/S",	VERTICALSPEED, 						2,7, SIGNED,	0,14).
		UI_VARIABLE("VREF",	"M/S",	VV_REF, 							2,7, SIGNED,	0,15).
		UI_VARIABLE("RREV",	"",		ROLL_REVERSAL_PARAM, 				2,7, SIGNED,	0,16).
	}
