...
FUNCTION MM6_BURN_TASK { PARAMETER DT.
	...
	LOCAL DT1 IS 5.0.
	LOCAL ALTITUDE0 IS SHIP:BODY:ALTITUDEOF(POSITIONAT(SHIP, EI_ESTIMATE - DT1)).
	LOCAL ALTITUDE1 IS SHIP:BODY:ALTITUDEOF(POSITIONAT(SHIP, EI_ESTIMATE + DT1)).
	LOCAL VELOCITY0 IS VELOCITYAT(SHIP, EI_ESTIMATE):ORBIT:MAG.
	SET EI_FLIGHTPATH TO ARCTAN((ALTITUDE0 - ALTITUDE1)/(VELOCITY0*2*DT1)).
...
FUNCTION MM6_REENTRY_TASK { PARAMETER DT.	
	...
	//
	// WHEN REACHING THE VELOCITY TARGET, START TRANSITION TO LEVEL FLIGHT.
	// AFTERWARDS TRANSFER TO APPROACH PHASE.
	//
	//	THIS DECISION IS MADE BASED ON OVERSHOOT THRESHOLD OR SIMPLY
	//	REACHING ZERO RESIDUAL VELOCITY.
	//
	IF (V < 100) OR (RUNWAY:DISTANCE < MM6_TARGET_DISTANCE*0.80) {
		SET TGT_ROLL_CMD TO 0.
		SET TGT_PITCH_CMD TO 0.
		
		LOCAL THRESHOLD IS 0.5.
		IF (ABS(ROLL_CMD - TGT_ROLL_CMD) < THRESHOLD) AND (ABS(PITCH_CMD - TGT_PITCH_CMD) < THRESHOLD) {
			TRANSFER_MODE(3,1).
		}
	}
...
FUNCTION MM6_UI_LO_TASK { PARAMETER DT.
	...
	LOCAL CM_LIMIT IS 7.409.
	LOCAL FLAP_POS IS 110.0.
...
IF MINOR_MODE >= 3 {
	...
	UI_VARIABLE("Q",		"KP",	SHIP:Q*100, 						3,5, NUMBER,	18,6).
	
	// GUIDANCE INFORMATION
	IF MINOR_MODE < 3 {
		// NONE
	} ELSE {
		UI_VARIABLE("DRAG",	"M/S2",	DRAG_CUR, 							2,7, SIGNED,	0,13).
		UI_VARIABLE("  VV",	"M/S",	VERTICALSPEED, 						2,7, SIGNED,	0,14).
		UI_VARIABLE("VREF",	"M/S",	VV_REF, 							2,7, SIGNED,	0,15).
		UI_VARIABLE(" REV",	"",		ROLL_REVERSAL_PARAM, 				2,7, SIGNED,	0,16).
	}
...
IF MINOR_MODE < 3 {
	...
	IF MINOR_MODE < 3 {
		UI_VARIABLE("  EI",	"SEC",	EI_ESTIMATE - TIME:SECONDS, 		1,7, NUMBER,	0,11).
		UI_VARIABLE("  -D",	"M",	ROUND(EI_DOWNRANGE/100,0)*100, 		1,7, NUMBER,	0,12).
		UI_VARIABLE("DRDT",	"M/S",	DR_DT, 								1,7, SIGNED,	0,13).
		UI_VARIABLE("  +H",	"M",	SHIP:BODY:ALTITUDEOF(POSITIONAT(SHIP,EI_ESTIMATE)), 0,7, NUMBER,	0,14).
		UI_VARIABLE(" PHI",	"M",	EI_FLIGHTPATH, 						2,7, SIGNED,	0,15).
		
		
	} ELSE {
		IF T < 1000.0 {
			UI_VARIABLE("  T1",	"SEC",	T, 									2,7, NUMBER,	0,11).
		} ELSE {
			UI_VARIABLE("  T1",	"SEC",	"-----",							0,7, TEXT,		0,11).
		}
		UI_VARIABLE("  T0",	"SEC",	T0, 								2,7, NUMBER,	0,12).
		
		UI_VARIABLE("A",	"DEG",	PITCH_CMD, 							2,6, SIGNED,	17,11).
		UI_VARIABLE("R",	"DEG",	ROLL_CMD, 							2,6, SIGNED,	17,12).
	}
...
FUNCTION MM6_TELEMETRY_TASK { PARAMETER DT.
	DOWNLINK("T0      ",ROUND(T0,2) ).
	DOWNLINK("T1",		ROUND(T,2) ).
	DOWNLINK("DRAG",	ROUND(DRAG_CUR,2) ).
	DOWNLINK("VV",		ROUND(VERTICALSPEED,2) ).
	DOWNLINK("VV_REF",	ROUND(VV_REF,2) ).
	DOWNLINK("CM",		ROUND(CM,3) ).
	DOWNLINK("ALPHA",	ROUND(PITCH_CMD,2) ).
	DOWNLINK("ROLL",	ROUND(ROLL_CMD,2) ).
	DOWNLINK("FLAP",	ROUND(SERVO_GET("BODY FLAP"),2) ).
