...
FUNCTION MAJOR_MODE3 {
	...
	SET HNAV_MOD TO MAX(0, MIN(1, (RUNWAY:DISTANCE - 20000)/25000 )).
	...
	// ENERGY SETPOINT
	SET SET_ENERGY TO ((MM3_TARGET_SPEED + 650*V_ENERGY)^2)/2 + 75000*MAX(0,MIN(1,RUNWAY:DISTANCE/20000)).
	SET TARGET_ENERGY TO MIN(TARGET_ENERGY, SET_ENERGY).
	IF RUNWAY:DISTANCE > 35000 { // ONLY MANAGE DYNAMIC PRESSURE
	...
	IF ALT:RADAR < MM3_FINAL_ALT { // FINAL FLARE CONTROL
		...
		SET VNAV_MOD TO MAX(0, MIN(1, (ALT:RADAR-MM3_GLIDE_ALT)/(MM3_FINAL_ALT*0.5) )).
		// FLARE CORRECTION
	...
	IF TIME:SECONDS - MODE_TIMER > 3.0 { // TRANSFER TO MODE 34 AFTER STABILIZING
		IF MINOR_MODE = 3 {
			TRANSFER_MODE(3,4).
		}
	}
	...
	IF (MINOR_MODE = 1) OR (MINOR_MODE = 2) { // DESCENT/APPROACH
		...
		// GENEREATE STEERING COMMANDS
		//SET TGT_HEADING_CMD TO HEADING_SIGN*(90 + HNAV).
		//SET TGT_PITCH_CMD TO -10 + VNAV.
		
		// RATE LIMIT HEADING
		//IF HEADING_CMD < TGT_HEADING_CMD {
		//	SET HEADING_CMD TO MIN(TGT_HEADING_CMD, HEADING_CMD + 8*DT).
		//}
		//IF HEADING_CMD > TGT_HEADING_CMD {
		//	SET HEADING_CMD TO MAX(TGT_HEADING_CMD, HEADING_CMD - 8*DT).
		//}

		// RATE LIMIT PITCH
		//IF PITCH_CMD < TGT_PITCH_CMD {
		//	SET PITCH_CMD TO MIN(TGT_PITCH_CMD, PITCH_CMD + 8*DT).
		//}
		//IF PITCH_CMD > TGT_PITCH_CMD {
		//	SET PITCH_CMD TO MAX(TGT_PITCH_CMD, PITCH_CMD - 8*DT).
		//}
	
		// STEERING LOOP
		//LOCK STEERING TO HEADING(HEADING_CMD, PITCH_CMD). //*R(0,0,-HNAV).
