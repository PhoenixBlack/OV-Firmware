...
FUNCTION MAJOR_MODE1 {
	...
	IF (MINOR_MODE = 5) OR (MINOR_MODE = 6) OR (MINOR_MODE = 7) { // MAIN GUIDANCE
		...
		IF SHIP:PERIAPSIS < -100000 { // FREEZE STEERING WHEN APPROACHING CUTOFF POINT
			SET STEERING_CONST TO 120000 - SHIP:APOAPSIS.
		} ELSE {
			// LIMIT THROTTLE WHEN FREEZING STEERING
			SET LIMIT_THROTTLE TO TRUE.
		}
