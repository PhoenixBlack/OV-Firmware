...
FUNCTION PRECALC_STAGE { PARAMETER S.
	...
	S:ADD("REF_VE", 9.81*S["REF_ISP"]).
	// FUEL FLOW (KG/SEC)
	S:ADD("REF_FF",	S["REF_F"]/(9.81*S["REF_ISP"])).
...
STAGES[1]:ADD("REF_F",		650000*0.25).					// ENGINE REFERENCE THRUST
...
FUNCTION MM4_READ_ACCELERATION {
	...
	RETURN 9.81*(PVAR_S2N(VALUE:SUBSTRING(0,VALUE:LENGTH-1))).
}
...
FUNCTION MM4_MAJOR_LOOP_TASK { PARAMETER DT.
	...
	IF DT <> INIT {
		...
		IF HAS_ACCELERATION_SENSOR { // CAN MAKE PRECISE ESTIMATE BASED ON ACCELERATION SENSOR
			...
			SET ESTIMATED_ISP TO ESTIMATED_THRUST/MAX(0.1, 9.81*ESTIMATED_FF).
		...
		IF (THRUST_PERCENTAGE > 0.70) AND (THRUST_PERCENTAGE < 1.30) AND
		   (ISP_PERCENTAGE    > 0.70) AND (ISP_PERCENTAGE    < 1.30) {
	...
	IF INCORPORATE_ESTIMATES {
		IF MINOR_MODE = 0 {
			SET STAGE1["F"] TO STAGE1["F"]*0.90 + 0.10*ESTIMATED_THRUST.
			SET STAGE1["VE"] TO STAGE1["VE"]*0.90 + 0.10*9.81*ESTIMATED_ISP.
			SET STAGE1["BURNTIME"] TO STAGE1["BURNTIME"]*0.10 + 0.10*(STAGE1["FUEL"]/ESTIMATED_FF)*1.01.
		}
	}
	...
	UI_VARIABLE("R   ", "M",		R0_0, 			0,8, NUMBER, 0,3).
	UI_VARIABLE("Rdot", "M/S",		V0_0, 			1,8, SIGNED, 0,4).
	UI_VARIABLE("-V  ", "M/S",		DV, 			1,8, NUMBER, 0,5).
	UI_VARIABLE("Rt", "",			TARGET_R, 		0,8, NUMBER, 18,3).
	UI_VARIABLE("Vt", "",			TARGET_V, 		0,8, SIGNED, 18,4).
	
	UI_VARIABLE("1   T", "",		STAGE1["T"], 	1,6, SIGNED, 0,7).
	UI_VARIABLE("1  A0", "",		STAGE1["A0"], 	1,6, SIGNED, 0,8).
	UI_VARIABLE("1 TAU", "",		STAGE1["TAU"], 	1,6, SIGNED, 0,9).
	UI_VARIABLE("2   T", "",		STAGE2["T"], 	1,6, SIGNED, 15,7).
	UI_VARIABLE("2  A0", "",		STAGE2["A0"], 	1,6, SIGNED, 15,8).
	UI_VARIABLE("2 TAU", "",		STAGE2["TAU"], 	1,6, SIGNED, 15,9).
	...
	IF MINOR_MODE = 0 { // STAGE 1
		UI_VARIABLE(" FF", "",		100*ESTIMATED_FF/STAGE1["REF_FF"], 			0,3, NUMBER, 22,11).
		UI_VARIABLE("THR", "",		100*ESTIMATED_THRUST/STAGE1["REF_F"], 		0,3, NUMBER, 22,12).
		UI_VARIABLE("ISP", "",		100*ESTIMATED_ISP/STAGE1["REF_ISP"], 		0,3, NUMBER, 22,13).
		UI_VARIABLE(" G", "",		ESTIMATED_A0/9.81, 							2,4, NUMBER, 22,14).
		UI_VARIABLE("CTA", "",		INCORPORATE_ESTIMATES, 						0,3, ONOFF, 22,16).
	}
...
FUNCTION MM4_MINOR_LOOP_TASK { PARAMETER DT.
	...
	// ATMOSPHERIC TEST
	IF ALTITUDE > 28000 { LOCK THROTTLE TO 0.25. }
	...
	LOCAL PITCH_MAX IS 87 - 27*PITCH_GRADIENT.
